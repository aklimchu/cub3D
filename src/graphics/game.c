...
/*   Updated: 2024/12/05 10:53:54 by aklimchu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void draw_game(t_cub *cub, float dist_to_ray, int ray_loop, float ray_angle)
{
	...
	float	wall_height = cub->map_size.x * cub->map_size.y * 320 / dist_to_ray;
	if (wall_height > 320)
		wall_height = 320;
	float	wall_offset = 160 - wall_height / 2;
	draw_line(cub->img_map, (t_coord_f){ray_loop * 8 + 750, wall_offset}, \
		(t_coord_f){ray_loop * 8 + 750, wall_height + wall_offset}, 0x008000FF);
}